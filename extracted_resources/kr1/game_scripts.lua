...
function scripts.tower_arcane_wizard.get_info(this)
	...
	local min = m.dps.damage_min * this.tower.damage_factor
	local max = m.dps.damage_max * this.tower.damage_factor
...
function scripts.soldier_barbarian.on_power_upgrade(this, power_name, power)
	if not this._on_power_upgrade_called then
		...
		this.ranged.attacks[2].animation = this.ranged.attacks[2].animation .. "2"
		...
		if this.unit.is_stunned then
			...
			if not a.disabled and store.tick_ts - a.ts >= a.cooldown then
				local towers = U.find_towers_in_range(store.entities, this.pos, a, function(t)
					return t.tower.can_be_mod and (not t.barrack or t.template_name == "tower_sorcerer" or t.template_name == "tower_spectres_mausoleum" or t.template_name == "tower_necromancer" or t.template_name == "tower_druid" or t.template_name == "tower_frankenstein" or t.template_name == "tower_time_wizard" or t.template_name == "tower_bone_flingers" or t.template_name == "tower_pixie" or t.template_name == "tower_mech") and t.template_name ~= "tower_grim_cemetery"
				end)
...
function scripts.enemy_demon_legion.update(this, store)
	...
	local function ready_to_clone()
		return this.enemy.can_do_magic and a.count > 0 and store.tick_ts - a.ts >= a.cooldown and P:nodes_to_defend_point(this.nav_path) > a.nodes_limit
	end
...
function scripts.enemy_necromancer.update(this, store)
	...
	local function ready_to_summon()
		if U.get_blocker(store, this) then
			...
			return this.enemy.can_do_magic and store.tick_ts - a.ts >= a.cooldown and not summon_count_exceeded()
		end
	end
...
function scripts.enemy_spectral_knight.insert(this, store)
	...
	return this.enemy.can_do_magic and true
end
...
function scripts.mod_ray_arcane.update(this, store)
	...
	local function apply_damage(value)
		...
		d.source_id = this.id
		d.target_id = target.id
		d.value = value * source.bullet.damage_factor
...
function scripts.mod_denas_tower.insert(this, store)
	...
	if this.range_factor then
		...
		if target.attacks.list[1].range then
		target.attacks.list[1].range = target.attacks.list[1].range * this.range_factor
		end
		if target.attacks.range then
		target.attacks.range = target.attacks.range * this.range_factor
		end
		end
		if target.barrack then
		if target.barrack.rally_range then
	...
	if this.cooldown_factor then
		...
		if target.attacks.list[1].cooldowns then
			local cd = target.attacks.list[1].cooldowns
			cd[1] = cd[1] * this.cooldown_factor
			cd[2] = cd[2] * this.cooldown_factor
		end
		end
...
scripts.tower_sunray_mini = {}

function scripts.tower_sunray_mini.get_info(this)
	local pow = this.powers.ray
...
function scripts.hero_voltaire.level_up(this, store, initial)
	...
	if l then
		...
		E:get_template(y.bullet.mod).duration = s.stun_duration[l]
...
function scripts.mod_ancient_guard.insert(this, store, script)
	...
	if target.melee then
		...
		if target.melee.attacks then
			if target.melee.attacks[1] then
				if target.melee.attacks[1].cooldown then
	...
	if target.template_name == "hero_denas" then
		target.timed_attacks.list[1].cooldown = target.timed_attacks.list[1].cooldown - this.increase
	end
...
scripts.bomb_airstrike = {}

function scripts.bomb_airstrike.update(this, store, script)
	local b = this.bullet
	...
	if b.level and b.level > 0 then
		if b.count_inc then
			b.count = b.count + b.level * b.count_inc
		end
	...
	if b.prehit_fx then
		...
		local sfx = E:create_entity(b.prehit_fx)
