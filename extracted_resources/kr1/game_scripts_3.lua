	...
	while true do
		if this.tower.blocked then
			...
			if store.tick_ts - ra.ts < ra.cooldown then
				...
				target = U.find_foremost_enemy(store.entities, this.pos, 0, a.range, false, ra.vis_flags, ra.vis_bans)
...
function scripts.tower_faerie_dragon.remove(this, store)
...
scripts.fireball_ashbite = {}

function scripts.fireball_ashbite.update(this, store)
	local b = this.bullet
	local mspeed = b.min_speed
	local tl = store.tick_length
	local ps
