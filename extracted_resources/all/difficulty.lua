...
function difficulty:patch_templates()
	...
	for _, t in pairs(E:filter_templates("enemy")) do
		...
		if t.timed_attacks then
			for i, a in ipairs(t.timed_attacks.list) do
				...
				PT(a, "max_range")
				PT(a, "generation")
				if a.allowed_templates_diff then
					PT(a, "allowed_templates")
				end
			end
		end

		if t.timed_actions then
			for i, a in ipairs(t.timed_actions.list) do
				PT(a, "count_group_max")
			end
		end

		PT(t, "power_block_duration")
		PT(t.demon, "speed")
		PT(t, "on_death_spawn_count")
	end
	...
	for _, t in pairs(E:filter_templates("bullet")) do
		...
		PT(t.bullet, "damage_type")
	end

	for _, t in pairs(E:filter_templates("graveyard")) do
		PT(t.graveyard, "spawns_by_health")
		PT(t.graveyard, "excluded_templates")
	end

	for _, t in pairs(E:filter_templates("cooldown_after")) do
		PT(t, "cooldown")
		PT(t, "cooldown_after")
	end
	
	for _, t in pairs(E:filter_templates("editor")) do
		PT(t, "spawn_groups")
	end
	
	for _, t in pairs(E:filter_templates("shield_ignore_hits")) do
		PT(t, "shield_ignore_hits")
	end
end
