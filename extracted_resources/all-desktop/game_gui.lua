...
function game_gui:init(w, h, game)
	...
	self.is_premium = true --PS.services.iap and PS.services.iap:is_premium()
...
function NotificationView:show(id, no_transition, force_show)
	...
	elseif n.layout == N_TOWER_2 then
		...
		if len == 2 then
			...
			views, pw, ph = create_layout(N_TOWER, n.images[1], n.prefixes[1], n.subs[1])
			local og_pos = {}

			local b_ok_t = create_noti_button("dark")
			b_ok_t.pos = V.v(475, 254)

			for i = 1, len do
				local photo = create_photo(n.images[i], 0, true)

				photo.pos.x, photo.pos.y = 145 + 150 * ((i - 1) % 4), 105 + 140 * math.floor((i - 1) / 4)
				photo.scale = V.v(0.75, 0.75)

				function photo.on_click(this)
					S:queue("GUINotificationOpen")

					views[2].text = _(n.subs[i])
					views[3].text = _(n.prefixes[i] .. "_NAME")
					views[4].text = _(n.prefixes[i] .. "_EXTRA")
					views[5]:set_image(n.images[i])

					views[1].pos.x = og_pos[1].pos.x + 75
					views[2].pos.x = og_pos[2].pos.x + 75
					views[3].pos.x = og_pos[3].pos.x + 75
					views[4].pos.x = og_pos[4].pos.x + 75
					views[5].pos = V.vclone(this.pos)
					b_ok_t.hidden = false
					for _, v in pairs(views) do
						v.hidden = false
					end

					timer:tween(0.4, views, og_pos)
				end

				self:add_child(photo)
			end

			function b_ok_t.on_click(this)
			...
			b_ok_t.hidden = true
			self:add_child(b_ok_t)

			for k, v in pairs(views) do
				v.hidden = true
				self:add_child(v)

				og_pos[k] = {pos = V.vclone(v.pos)}
			end

			if len == 3 then
				b_ok.pos = V.v(450, 170)
				self.size = V.v(595, 150)
			else
				local th = 150 + 140 * math.floor((len - 1) / 4)
				b_ok.pos = V.v(600, len % 4 == 0 and th + 20 or th - 120)
				self.size = V.v(745, th)
			end
			self.anchor = V.v(self.size.x / 2, self.size.y / 2)
		end
